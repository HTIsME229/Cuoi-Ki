...
arrow_episode.onclick = function () {
  ...
  if (translateValue === -150) arrow_episode.style.opacity = "0";
};
...
let arrow_movieSimilar = document.querySelector(
  ".list-similarMovie .click-ahead"
);

let similarMovie_list = document.querySelectorAll(
  ".list-similarMovie .for-similar"
);
...
arrow_movieSimilar.onclick = function () {
  if (translateValue_movieSimilar > -50) {
    ...
    for (let i = 0; i < similarMovie_list.length; i++) {
      similarMovie_list[
        i
      ].style.transform = `translateX(${translateValue_movieSimilar}%)`;
    }
    if (translateValue_movieSimilar === -50) {
...
arrow_movieSimilarBack = document.querySelector(
  ".list-similarMovie .click-back"
);
