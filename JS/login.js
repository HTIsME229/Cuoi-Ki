...
signUp.addEventListener("click", function () {
  ...
  let pass_reg = document.querySelector(
    ".box_signUp .password .content-input"
  ).value;

  let mail_reg = document.querySelector(
    ".box_signUp .email .content-input"
  ).value;
  if (mail_reg != "" && pass_reg != "" && check_box) {
    box_login.style.display = "block";
    box_button.style.display = "none";
    box_signup.style.display = "none";
  } else if (mail_reg == "") alert(`Nhap Mail`);
  else if (pass_reg == "") alert(`Nhap Password`);
  else alert(`Xác nhận điều khoản`);
});
...
document.querySelector(".login").onclick = () => {
  let username = document.querySelector(".login__info .content-input").value;
